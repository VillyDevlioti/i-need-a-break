...
class VideoHolder extends Component {
    ...
    selectRandomAnimation = (playNextVideo) => {
        ...
        if (playNextVideo) { // has the button been clicked? If yes
            randomChoice = (Math.random() * ceiling) + 1; //assign a new random choice for the video
            nextVideo = animations[randomChoice].source; //pick the next video
    ...
    render () {
        ...
        return (
            <Row className="video-row video-text">
                ...
                {/*<ShuffleButton />*/}
                {/*Shuffle Button Code*/}
                <Button type="button" className="btn btn-outline-primary shuffle-button">
